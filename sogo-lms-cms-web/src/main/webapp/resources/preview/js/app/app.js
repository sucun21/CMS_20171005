var app = angular.module('app', ['ngSanitize', 'angular-cache']);

app.filter('genVideoTag', ['$sce', function ($sce) {
	return function (val) {
		if (val) {
...
app.filter('activityTime', function () {
...
function catalougePreviewCtrl($scope, $rootScope, $q, dataService, $timeout, $sce) {
	var vm = this;
	vm.activityTypeId = 0;
	vm.isDoneQuiz = false;
    ...
    var getActivityDetails = function(activityId){
		...
		var promise = dataService.getActivityDetails(activityId);
    ...
    $scope.init = function(unitId, lessonId, activityId){
    	$q.all([
    		...
    		if(vm.activityTypeId == 4){ //quiz
    			var promise = dataService.getQuizQuestion(vm.activityId);
    			promise.then(function(res){
...
catalougePreviewCtrl.$inject = ['$scope', '$rootScope', '$q', 'dataService', '$timeout', '$sce'];

app.controller('catalougePreviewCtrl', catalougePreviewCtrl);