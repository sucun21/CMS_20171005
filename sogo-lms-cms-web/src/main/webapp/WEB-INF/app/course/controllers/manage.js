(function () {
    ...
    angular.module('app').controller('Course.Manage', 
    	...
    	angular.forEach(vm.listCourse, function(item){
    		...
    		courseService.getUnitInCourse(params).then(function(res){
      ...
      vm.saveTitle = function(course, e){    	  
    	  if(!e || (e && (e.charCode == 13 || e.keyCode == 13 || e.which == 13))){
    		  //save
    		  if(course.courseTitle.length > 250){
    			  notificationService.error('Course title too long.(Max 250 chars)');
    		  }
    		  else{
      ...
      vm.saveShortDescription = function(course){
    	  if(course.courseTitle.length > 250){
			  notificationService.error('Short description too long.(Max 2000 chars)');
		  }
		  else{
      ...
      vm.addNewUnit = function(course){
    	  progressService.start();
    	  var defaultUnitTitle = 'New Unit';
    	  var params = {
    		courseId: course.courseId,
    	  ...
    	  courseService.executeUnitCourse(params).then(function(res){    		  
    		  if(res && res.data.id > 0){
    			  notificationService.success('Unit added');
    			  
    			  course.listUnit = course.listUnit ? course.listUnit : [];
    			  
    			  course.listUnit.push({
					 unitId: res.data.id,
					 unitTitle: defaultUnitTitle
				  });
    		  }
    		  else{
    			  notificationService.error('Error occur');
    		  }
    		  progressService.complete();
    	  });
      };
      
      vm.saveUnitTitle = function(unit, e){
    	  if(e && (e.charCode == 13 || e.keyCode == 13 || e.which == 13)){
    		  //save
    		  if(unit.unitTitle.length > 250){
    			  notificationService.error('Unit title too long.(Max 250 chars)');
    		  }
    		  else{
    			  progressService.start();
    			  var params = {
		    	  ...
		    	  courseService.executeUnitCourse(params).then(function(res){
		    		  if(res && res.data.id > 0){
		    			  notificationService.success('Unit updated');
		    		  }
		    		  else{
		    			  notificationService.error('Error occur');
		    		  }
